...
| G3 | SDS §1: `multer` file uploads to `/uploads`; SRS F2 lists Photo | **No upload endpoint.** `photoUrl` is a URL string validated by `z.string().url()` | Photo must be a URL input, not a file picker. A real upload needs a backend endpoint first |
